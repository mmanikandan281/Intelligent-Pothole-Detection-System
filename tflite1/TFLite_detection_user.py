...
def check_location_and_start_detection():
    ...
    if from_location.strip() == "" or to_location.strip() == "":
        ...
        clear_existing_widgets()
        start_detection(from_location, to_location, video_file_path)  # Pass the from, to locations, and video file path

video_file_path = "C:\\Users\\MANIKANDAN.M\\Desktop\\tflite1\\test.mp4"  # Replace this with the actual path to your video file
def start_detection(from_location, to_location, video_file_path):
    ...
    imW = int(videostream.get(cv2.CAP_PROP_FRAME_WIDTH))
    imH = int(videostream.get(cv2.CAP_PROP_FRAME_HEIGHT))
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                ...
                if object_name == "Pothole":
                    ...
                    send_email(image_path, from_location, to_location)  # Pass from and to locations
...
def send_email(image_path, from_location, to_location):
    global capture_count  # Assuming capture_count is a global variable

    sender_email = "manikandanmskc015@gmail.com"  # Your email address
    receiver_emails = ["mmanikandan281@gmail.com", "maneeshem27@gmail.com"]  # List of receiver email addresses
    password = "swzk ikga ydpl cewf"  # Your email password
    ...
    text = f"Pothole has been detected From: {from_location} To: {to_location}. Please act accordingly."
    html = f"""\
    <html>
      <body>
        <p>Pothole has been detected From: {from_location} To: {to_location}. Please act accordingly.</p>
        <img src="cid:image1">
      </body>
    </html>
    """
